...
import { exportAssetsToJson, ExportAssetsToJsonParams } from "../exportAssetsToJson";
...
test('should download both entities and relationships when specified to include them', async () => {
  ...
  expect(bulkDownloadToJson).toHaveBeenCalledWith(expect.objectContaining({
  ...
  expect(mockedSpinner.succeed).toHaveBeenCalledWith(expect.stringMatching(
...
test('should log error when there is an issue bulk downloading', async () => {
  ...
  expect(mockedSpinner.fail).toHaveBeenCalledWith('Failed to export assets to JSON');
});
