...
const TEST_FILES = {
  [`${TEST_DIRECTORY}/json/entities/entity_type_1/${uuid()}.json`]: JSON.stringify(
    [createEntity({ id: '1', type: '1' })]
  ),
  [`${TEST_DIRECTORY}/json/entities/entity_type_1/${uuid()}.json`]: JSON.stringify(
    [createEntity({ id: '2', type: '1' })]
  ),
  [`${TEST_DIRECTORY}/json/entities/entity_type_2/${uuid()}.json`]: JSON.stringify(
    [createEntity({ id: '3', type: '2' })]
  ),
  [`${TEST_DIRECTORY}/json/entities/entity_type_2/${uuid()}.json`]: JSON.stringify(
    [createEntity({ id: '4', type: '2' }), createEntity({ id: '5', type: '2' })]
  ),
  [`${TEST_DIRECTORY}/json/relationships/relationship_type_1/${uuid()}.json`]: JSON.stringify([
    createRelationship({ from: createEntity({ id: '1', type: '1' }), to: createEntity({ id: '3', type: '2' }) }),
    createRelationship({ from: createEntity({ id: '2', type: '1' }), to: createEntity({ id: '3', type: '2' }) })
  ]),
  [`${TEST_DIRECTORY}/json/relationships/relationship_type_2/${uuid()}.json`]: JSON.stringify([
    createRelationship({ from: createEntity({ id: '4', type: '2' }), to: createEntity({ id: '2', type: '1' }) }),
  ]),
};
...
test('should export both entities and relationships when specified', async () => {
  ...
  const assets = await Promise.all(fetchAssetsContents(vol).map(convertCsvToJson))
...
test('should log error when export fails', async () => {
  ...
  await expect(exportJsonAssetsToCsv({
    includeEntities: true,
    includeRelationships: true,
    storageDirectory: TEST_DIRECTORY,
    apiKey: TEST_API_KEY
  ...
  expect(mockedSpinner.fail).toHaveBeenCalledWith('Failed to export JSON assets to CSV');
});
