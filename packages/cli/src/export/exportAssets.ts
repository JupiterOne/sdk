...
export type ExportAssetsParams = Omit<ExportOptions, 'dataDir'> & { storageDirectory: string };

export default async function exportAssets(options: ExportAssetsParams) {
  log.info(`Starting account export to the [${options.storageDirectory}] directory`);
