import { createIntegrationInstanceForLocalExecution, prepareLocalStepCollection } from '@jupiterone/integration-sdk-runtime';
  ...
  try {
    ...
    await import(`${directory}/src/index`).then(
      async ({ invocationConfig }) => {
        await invocationConfig.validateInvocation({instance: createIntegrationInstanceForLocalExecution(config)},);
    },
    );
  }
  catch(err) {
